...
```{r setup}
  ...
  sh_base <- st_read("./data/gpkg/asc_shbase.gpkg") %>% 
      st_transform(crs = 4283)
...
coast <- st_transform(coast, crs = st_crs(bch))
bch <- dat %>% filter(Activity %in% "Beachgoing") # filter to beach going

bch <- st_snap(bch, coast) # snap to coast
...
#'[#NOTE:Check empty grids before removing]  
  ...
  ggplot() +
    geom_sf(data = coast) +
    geom_sf(data = sh_base, aes(colour = zone), lwd = ) +
  geom_sf(data = bch, size = 0.5)
```
