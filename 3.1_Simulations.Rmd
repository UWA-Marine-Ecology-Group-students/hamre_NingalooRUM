...
a <- nrow(b) + 2
v <- read_excel("./data/03_data/attmod.xlsx", sheet = "v", col_names = rep("x", a)) %>% # covariance covarience matrix
  dplyr::select(-x...1 ) %>%
  distinct()
...
# attmod_base: ChARLOTTE
# attmod: acs vs att fine scale attribute model
b <- read_excel("./data/03_data/attmod.xlsx") %>% dplyr::select(Vars, Coef)

a <- nrow(b) + 2
v <- read_excel("./data/03_data/attmod.xlsx", sheet = "v", col_names = rep("x", a)) %>%
  dplyr::select(-x...1 ) %>%
  distinct()
```
# Rearrange data
