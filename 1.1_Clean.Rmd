...
dat <- dat %>%
  mutate(Date_posix = as.POSIXct(ifelse(is.na(nDate), CreationDate.x, nDate), 
                                 origin = '1970-01-01', tz = "Australia/Perth") + 8*60*60) %>% 
  mutate(nDate_posix = as.POSIXct(nDate, origin = '1970-01-01', tz = "Australia/Perth") + 8*60*60) %>% 
  mutate(Date_posix = ifelse(is.na(nDate_posix), CreationDate.x, nDate)) %>% 
  mutate(jTime = ifelse(is.na(jTime) & !is.na(jDate), "14:00", jTime)) %>% 
  mutate(jDate_posix = paste(jDate, jTime, sep = (" "))) %>% 
  mutate(jDate_posix = as.POSIXct(jDate_posix, format = "%d/%m/%Y %H:%M",origin = '1970-01-01', tz = "Australia/Perth")) %>% 
  mutate(Date_posix = as.POSIXct(ifelse(is.na(Date_posix), as.POSIXct(jDate_posix), as.POSIXct(Date_posix)), 
                                 origin = '1970-01-01', tz = "Australia/Perth")) %>% 
  filter(!is.na(Date_posix)) %>% 
  mutate(Time = format(Date_posix, "%p")) %>% 
  mutate(SurveyDate = as.Date(Date_posix)) %>% 
...
dat <- dat %>%
  ...
  mutate(FishingType = str_trim(FishingType)) %>% 
  ...
  dplyr::mutate(FishingType = ifelse(jFishingType %in% c("Demersal","Demersal","Bottom-bashing (drifting)","Bottom-bashing (anchored)"), "Demersal", FishingType)) %>%
...
dat <- dat %>%
  ...
  dplyr::mutate(KeptUndam = ifelse(numYear < 2017, CaughtUndam, KeptUndam)) %>% 
...
### Total number of fish hooked (undamaged and depredated)s
```{r nHooked}
 dat <- dat %>%
...
dat <- dat %>% 
  ...
  mutate(exTimes12m = ifelse(exTimes12m %in% "50100", "75", exTimes12m)) %>% 
...
dat <- dat %>% 
  dplyr::select(
    ID, TripID, SurveyDate, Date_posix, Time, numYear, facYear, Interviewer, mLat, mLong, PersonID, Screen18,
