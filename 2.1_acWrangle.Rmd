...
# Boat Length - need to fill in NAs where possible
...
```{r message=FALSE}
# Depth
tmp <- dat %>% dplyr::select(TripID, gridID_alt, asc_geom) %>% st_as_sf() 
tmp <- mean_depth(bathy, tmp) # get average depth of grid
tmp %<>% as.data.frame() 
...
dat %<>%
  ...
  #extracting swell
   hsws <- get_hs_ws_day(dat$TripDate, dat$Long, dat$Lat) # getting hs and ws
...
# SST
  coast_lambert <- coast %>% st_transform(crs = 3112) # need land lambert shp file loaded

  sst_kelvins <- get_sst(Dates = dat$TripDate, Long = dat$Long, Lat = dat$Lat) # getting sst
  
  sst_celcius <- sst_kelvins - 273.15 # convert to celcius
  
  dat$sst <- sst_celcius

  # tetsing new function
  t <- dat %>% distinct(TripDate, .keep_all = TRUE) %>% filter(TripID %in% c(34, 36, 42))
  
  sst_kelvins <- get_sst(Dates = t$TripDate, Long = t$Long, Lat = t$Lat) # getting sst
```
