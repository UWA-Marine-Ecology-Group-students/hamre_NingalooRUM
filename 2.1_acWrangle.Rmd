...
# mp_stats
    ...
    #'[#TODO: add this validation plot to ascgrid function, maybe make extra function to remove those zones from model? add warning if there is no data in grid cell]  
    
    grid$empty <- ifelse(grid$data_present == 0 & grid$sz == 0, 1, 0)# shows row number/gridID_alt of cell missing dat
    
    ggplot()+
      geom_sf(data = grid, aes(fill = as.factor(grid$empty))) # check which one it is
    
     # grid <- grid %>% filter(gridID_alt != 25) # remove
    

    empty <- grid %>% filter(empty == 1) # binary emppty col
    
    empty_int <- st_intersects(empty, grid) # 
    
    st_rook = function(a, b = a) st_relate(a, b, pattern = "F***1****") # find nb function
    
    grid2 <- grid %>% mutate(empty_nb = st_rook(.)) # find all nb in grid
    
    grid2 %<>% filter(empty == 1) # get only ones which are no empty
  

test <- grid %>% filter(gridID_alt %in% c(empty_int[[1]], empty_int[[2]], empty_int[[3]])) # get those in list

t2 <- test %>% filter(empty == 0, sz == 0) # filter to get applicable nb

t2ID <- unique(t2$gridID_alt) # isolate ids of applicable neighbours


# need to change grid iod of emoty one to match id in t2ID which is in empty_nb column 

# need 
t3 <- grid2 %>% 
  mutate(empty_nb2 = ifelse(empty_nb %in% t2ID, "YES", "NO"))
...
ggplot() +
  ...
  geom_sf(data = t2, fill = "red")
  geom_sf(data = empty_int, fill = "blue")
  
  t3[, c("empty_nb", "empty_nb2")]
```
