...
```{r setup}
    ...
    grid_base <- st_read("./data/gpkg/sims/ascBase_v3.gpkg") %>% 
      st_transform(crs = 4283)
    ...
    # simulation
    sim_name <- "sim3"       #'[#NOTE: check simulation name]
    # 
    # sim <- st_read(paste0("./data/gpkg/sims/asc_", sim_name, ".gpkg")) %>% 
    #   st_transform(crs = 4283)
    
    sim <- st_read(paste0("./data/gpkg/ning_sim+sz.gpkg")) %>% 
      st_transform(crs = 4283)
...
Need the current sz to make the grid which doesnt intersect wit zones, but these are later removed because the have no recreational fishing value (if doing extractive rums only).
```{r message = FALSE}
# # asc grid
#     grid <- ASCgrid(poly = grid_base, sz_current = npz, sz_sim = sim, point = dat, from = 0.03,
#                     by = 0.02, crs = 4283, vert = FALSE)
# 
# # Proposed needs manual edits: merge small port areas, removed tiny little polygons around islands  - idetified through centroid plot in next chunk - chek to make sure none lok out of place
# #
#     grid %<>% st_transform(crs = 4283)
#     st_write(grid,
#              paste0("./data/gpkg/sims/2.1_asc_manual_edit_", sim_name, "_4283.gpkg"),
#              append = FALSE) # need to write with st_write to open in QGISRUMs

    grid <- st_read(paste0("./data/gpkg/sims/2.1_qgis_asc_dem_manual_edit.gpkg")) %>%
      st_transform(crs = 4283) %>% 
  st_make_valid()
  #
    # making grid columns
    grid %<>% dplyr::select(-contains("gridID_"))

    grid %<>% mutate(gridID_alt = row_number())

    for (i in unique(grid$gridID_alt)) {
    gridID = ifelse(grid$gridID_alt == i, 1, 0)
    grid[,length(grid) + 1] <- gridID
    colnames(grid)[ncol(grid)] <- paste0("gridID_", i)
  }

    # needs redone after manual manipulation
    grid$area <- as.numeric(round(set_units(st_area(grid), km^2), 2))
    grid$use_count <- lengths(st_intersects(grid, dat))

  #'[#NOTE:Check empty grids before removing]  
  # Remove current sz and cells with no data presnet
    grid %<>% filter(sz_current == 0 & use_count != 0)

  # plot should have no zeros and check data to make sure
    ggplot() +
      geom_sf(data = grid, aes(fill = as.factor(use_count))) +
      geom_sf(data = dat, size = 0.5)
    
    # if there are any points outwith grid probs in sz - check before removing
    
    #nrow(dat): 543
    dat %<>% mutate(to_rm = ifelse(st_intersects(dat, grid), 1, 0)) %>% 
      filter(!is.na(to_rm))
    
    ggplot() +
      geom_sf(data = grid, aes(fill = as.factor(sz)))+
      geom_sf(data = dat, size = 0.5)
```
...
hab_grid[2:4] <- hab_grid[2:4] - 1 # dont know why function above is summing and adding 1 so no zeros exist. 
...
grid %<>% 
  mutate(tant = ifelse(as.list(st_intersects(grid, tant)) %in% "integer(0)", NA, "T"))

grid %<>% 
  mutate(CB = ifelse(as.list(st_intersects(grid, CB)) %in% "integer(0)", NA, "CB"))

grid %<>%
  mutate(launch = ifelse(tant %in% "T", "T", NA)) %>% 
  mutate(launch = ifelse(CB %in% "CB", "CB", launch)) %>% 
  mutate(launch = ifelse(is.na(launch), "G", launch))

ggplot() +
  geom_sf(data = grid, aes(fill = as.factor(launch)), lwd = 0.05) + theme_classic()
```
   ...
   #   dplyr::select(-gridID_alt.y)
    ...
    dat %<>%
      ...
      dplyr::select(TripID, PersonID, gridID_vis, Site, TripDate, MedianTime, BoatLength, cost, ipw, Postcode, 
                    ...
                    KeptUndam, CaughtUndam,Dem, Dem_bin, dem_only, Start_hr, BaitLure, Age, exTimes12m, TripMonth, numYear, 
