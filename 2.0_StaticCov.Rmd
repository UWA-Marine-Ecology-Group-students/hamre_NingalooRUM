```{r setup}
    ...
    ext <- st_read("./data/gpkg/gridextent.gpkg") # study site (mega mvm polygon)
...
```{r}
    ...
    grid$centroid <- centroid$x # bind centroid to grid sf 
    
    ggplot() +
      geom_sf(data = grid$centroid, size = 0.05)
```
...
# grid <- habitat_area(sf_habitat = habitat, col_habitat = 1, sf_grid = grid, id_grid = gridID) # need to update updated pivot wioder from spread
  ...
  grid <- grid %>% 
    mutate(Pelagic = ifelse(is.na(Pelagic), area, Pelagic)) %>% 
    mutate_at(c("Lagoon", "Reef", "RReef"), ~replace_na(.,0))
```
## Land
...
# checkpoint
ggplot() +
  geom_sf(data = grid, aes(fill = km_mainland))

ggplot() +
  geom_sf(data = grid, aes(fill = isl_adj))
```
## Dist to BR
...
gridID <- grid$gridID
dist$gridID <- gridID
dist <- dist %>% gather("RampID", "km_BR", 1:length(dist) - 1) 
