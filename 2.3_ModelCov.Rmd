...
full_dat <- readRDS("./data/gpkg/sims/2.1_dat.gpkg") %>% mutate(rug = as.numeric(rug))
```
...
pred.vars <- c("Dem", "depth", "l_km_br", "nl_km_br", "pelagic", "reef", "rreef", "lagoon", "decDuration", "rug", "centroidLat") 
...
# Plot transformations - need to run in console
  ...
  dat$log_dem <- log(dat$Dem + 1)
  dat$log_depth <- log(dat$depth + 1)
  dat$log_lkm_br <- log(dat$l_km_br + 1)
  dat$log_nlkm_br <- log(dat$nl_km_br + 1)
  ...
  # update predictors
pred.vars <- c("log_depth","log_nlkm_br","pelagic", "log_reef", "log_rreef", "log_lagoon", "sqrt_decDuration", "sqrt_rug", "centroidLat") 
```
...
# test model
test_fit <- gam(Dem ~ s(log_depth, k = 3, bs = 'cr') + s(PersonID, bs = 're'), family = tw(), data = dat)
...
topmod <- gam(Dem ~ 
                ...
                s(log_lagoon, k = 3, bs = 'cr') +
                s(sqrt_rug, k = 3, bs = 'cr') + 
                # s(pelagic, k = 3, bs = 'cr') + 
...
```{r}
 # Make transformations
  ...
  full_dat$log_lagoon <- log(full_dat$lagoon + 1)
  # full_dat$sqrt.decDuration <- sqrt(full_dat$decDuration)
  full_dat$sqrt_rug <- sqrt(full_dat$rug)
...
full_dat %<>% rename(pfit_dem = p_dem.fit,
                     pse_dem = p_dem.se.fit) 

# Check summary
summary(full_dat$pfit_dem)
```

## Predictor plots
```{r}
# define top mod predictors
# top.preds <- c("log.depth", "log.lagoon", "sqrt.decDuration", "centroidLat", "sqrt.rug")
# 
...
# 
# ggplot(data = grids, aes(x = sqrt.decDuration, y = "p_dem.fit")) +
#     ylab("Predicted demersal catch") +
#     geom_smooth(alpha = 0.75, size = 2, show.legend = FALSE) +
#     theme_classic()
```
...
grid <- full_dat %>% distinct(gridid_alt, .keep_all = TRUE) %>%
  st_as_sf()  

ggplot() +
  geom_sf(data = grid$grid_geom, aes(fill = grid$p_dem.fit), lwd = 0.05) +
  scale_fill_distiller(palette = "YlGnBu", direction = 1) +
  labs(fill = "Demersal\nCatch") +
  theme_classic()

ggplot() +
  geom_sf(data = grid$grid_geom, aes(fill = grid$p_dem.se.fit), lwd = 0.05) +
  scale_fill_distiller(palette = "YlGnBu", direction = -1) +
...
full_dat %<>% dplyr::select(-c(br_geom, dat_geom, grid_geom)) # data for stata
