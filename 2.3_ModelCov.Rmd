...
resp <- dat$caught_bin
# resp <- dat$Dem_bin 
# dat$Dem_bin <- as.factor(dat$Dem_bin)
...
```{r}
  ...
  # dat$PersonID <- as.factor(dat$PersonID)
  
  
  # full_dat$log_depth <- log(full_dat$depth + 1)
  # full_dat$PersonID <- as.factor(full_dat$PersonID)
...
pred.vars <- c("depth", "reefs", "lagoon", "rug", "MedianHour", "centLat") # ASC
...
test_fit <- gam(KeptUndam ~ s(depth, k = 3, bs = "cr") + s(facYear, bs = 're'), 
...
full_dat <- cbind(full_dat,
                "p_dem" = predict(topmod, newdata = full_dat, se.fit = T))
...
ggplot() +
  ...
  theme_classic()
# 
# a <- full_dat %>% filter(p_dem.fit > 69)
# a[, c("TripID","gridID_alt","Dem", "p_dem.fit", "centLat", "MedianHour", "lagoon", "log_depth", "facYear")] 
# 
# summary(a$p_dem.fit)
# table(a$p_dem.fit)
# 
# a[which(a$p_dem.fit %in% c(19, 27)), ]
```
...
east <- full_dat %>%  filter(Site %in% c("BundegibR", "ExmouthBR")) %>% filter(TripID == 880)

trips <- c(2, 880, 1136, 1353)

grid <- full_dat %>% 
  filter(TripID == 880) %>%
  st_as_sf()

bigcatch <- grid %>% filter(gridID_alt %in% unique(a$gridID_alt))

points <- full_dat %>% filter(Site %in% c("TantabiddiBR", "CoralBayBR")) %>% distinct(TripID, dat_geom)

ggplot()+
  geom_sf(data = grid$asc_geom, lwd = 0.05)+
  geom_sf(data = bigcatch$asc_geom, fill = "blue") +
  geom_sf(data = points$dat_geom, size = 0.25) +
  theme_classic()

ggplot() +
  geom_sf(data = grid$asc_geom, aes(fill = as.factor(grid$p_dem.fit)), lwd = 0.05) +
...
a <- full_dat[which(full_dat$p_dem.fit > 10), ]
a

dat %>% group_by(gridID_alt) %>% 
  summarise(av = mean(CaughtUndam > 0))
  # summarise(av = mean()) %>% 
ggplot() +
  geom_sf(data = dat$asc_geom, aes(fill = as.factor(dat$av)), lwd = 0.05) +
  # scale_fill_distiller(palette = "YlGnBu", direction = 1) +
  labs(fill = "Demersal\nCatch") +
  theme_classic()
