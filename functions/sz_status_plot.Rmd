...
access %<>% mutate(access = ifelse(id == 17, 3, access))
...
ggplot(sz, xmin = 0, ymin = 0, ylim = 20) +
  ...
  scale_fill_manual(
          values = c("0" = "white", "1" = "lightblue", "2" = "dodgerblue3", "3" = "darkblue"),
          name = "Shore Based\n Human Connection\n Potential") +
  # geom_text(data = filter(sz, shore > 4.5 & area > 4.5),
  #           aes(x = shore, y = area, label = Name), size = 2.5) +
   theme(plot.margin = margin(1,1,1.5,1.2, "cm"),
         legend.box = "horizontal",
         legend.title.align = 0.5) +
   ylim(min = 0, max = max(sz$area) + 50) +
   xlim(min = 0, max = max(sz$shore) + 5) +
  labs(y = bquote('Area'~(km^2)), x = "Shore Protection (km)")
