...
# Make data stata friendly
    ...
    b_ex_dat %<>% 
      dplyr::select(trip_id, person_id, gridid_vis, ramp_name, ipw, cost, ipw, boat_access,
                    ...
                    accom, geom_accom, geometry, use_long, use_lat) %>%
      distinct() %>% 
      as.data.frame()
...
## Zone
```{r}
rum_dat$cset <- substr(rum_dat$gridid_alt, 1, 3) # making choice set col so i can plot them separately

rum_dat %<>% 
  mutate(b_sz = ifelse(cset %in% c("b_e", "b_n") & sz == 1, 1, 0)) %>% 
  mutate(s_sz = ifelse(cset %in% c("s_e", "s_n") & sim_zone %in% "sz", 1, 0)) %>% 
  mutate(sz = ifelse(b_sz == 1 | s_sz == 1, 1, 0)) # this is both current and simn

rum_dat %<>% 
  mutate(zone_mod = ifelse(str_detect(gridid_alt, "nex") & sz == 1, 1, 0)) # zone for model
