...
dat[which(dat$Activity %in% "Other"), ]
```
...
# st_write(b_ex, paste0("./data/gpkg/sims/2.1_asc_manual_edit_", g1, "_", sim_name, "_4283.gpkg"), append = FALSE) # need to write with st_write to open in QGISRUMs
   ...
   b_ex <- st_read(paste0("./data/gpkg/","b_ex_grid_", sim_name, "_qgis_v2.gpkg")) %>%
      st_transform(crs = 4283) %>% st_make_valid()
```
...
b_ex_dist %<>%
  ...
  b_ex %<>% 
    rename(geom_asc = geom, geom_cent = centroid)
...
##'[#NOTE:Check empty grids before removing]
  ...
  # Remove current sz and cells with no data presnet
    ...
    b_ex %<>% mutate(sz_current = ifelse(is.na(sz_current), 0, sz_current))
    b_ex %<>% mutate(sz = ifelse(is.na(sz), 0, sz))
    
    b_ex %<>% filter(sz_current == 0)
  ...
  ## offshore/inshore vraiable
   ...
   #   geom_sf(data = offshore, fill = "red")
    ...
    b_ex %<>% filter(!(gridid_alt %in% "b_ex_48")) # rogue line
 
    a <- b_ex %>% filter(use_count == 0 )
 
    ggplot() +
   # geom_sf(data = b_ex$geom_asc) +
   geom_sf(data = b_ex$geom_asc, aes(fill = as.factor(b_ex$use_count)))
...
##'[#NOTE:Check empty grids before removing]  
  ...
  # Remove current sz and cells with no data presnet
   ...
   ggplot()+
    ...
    ## adding conglometrated b_nes set
    
    b_nex %<>% 
      mutate(gridid_alt2 = ifelse(str_detect(gridid_alt, "b_nex") & br_site %in% "e", 
                              paste("b_nex2", 1, sep = "_"), NA)) %>% 
      mutate(gridid_alt2 = ifelse(str_detect(gridid_alt, "b_nex") & br_site %in% "n_w", 
                              paste("b_nex2", 2, sep = "_"), gridid_alt2)) %>% 
      mutate(gridid_alt2 = ifelse(str_detect(gridid_alt, "b_nex") & br_site %in% "s_w", 
                              paste("b_nex2", 3, sep = "_"), gridid_alt2))

    
   b_nex %<>%
     mutate(gridid2_b_nex_1 = ifelse(gridid_alt2 %in% "b_nex2_1", 1, 0)) %>% 
     mutate(gridid2_b_nex_2 = ifelse(gridid_alt2 %in% "b_nex2_2", 1, 0)) %>% 
     mutate(gridid2_b_nex_3 = ifelse(gridid_alt2 %in% "b_nex2_3", 1, 0)) 
   
   
    b_nex_grid <- b_nex %>%
      dplyr::select(gridid_alt2, geom_asc) %>%
      distinct()

    # allocating sites to grids
    b_nex_dat %<>% 
      st_join(b_nex_grid, left = T, join = st_intersects) %>% 
      rename(gridid_vis2 = gridid_alt2)
```
...
# calculating length
  ...
  s_nex_grid <- s_nex_buff %>%
    ...
    s_nex_buff %<>% 
      mutate(gridid_alt2 = ifelse(str_detect(gridid_alt, "s_nex") & br_site %in% "e", 
                              paste("s_nex2", 1, sep = "_"), NA)) %>% 
      mutate(gridid_alt2 = ifelse(str_detect(gridid_alt, "s_nex") & br_site %in% "n_w", 
                              paste("s_nex2", 2, sep = "_"), gridid_alt2)) %>% 
      mutate(gridid_alt2 = ifelse(str_detect(gridid_alt, "s_nex") & br_site %in% "s_w", 
                              paste("s_nex2", 3, sep = "_"), gridid_alt2))

    
   s_nex_buff %<>%
     mutate(gridid2_s_nex_1 = ifelse(gridid_alt2 %in% "s_nex2_1", 1, 0)) %>% 
     mutate(gridid2_s_nex_2 = ifelse(gridid_alt2 %in% "s_nex2_2", 1, 0)) %>% 
     mutate(gridid2_s_nex_3 = ifelse(gridid_alt2 %in% "s_nex2_3", 1, 0)) 
...
# Make data stata friendly
    ...
    b_nex_dat %<>% 
      dplyr::select(trip_id, person_id, gridid_vis, gridid_vis2, ramp_name, ipw, cost, ipw, boat_access,
                    cons_flt, sp_flt, cons2l, sp2l, cons3l, sp3l, gam_cons, gam_sp, vott, ann_inc, age,
                    ex_times12m, resident,accom, geom_accom, geometry, use_long, use_lat) %>%
      distinct() %>% 
      as.data.frame()
    ...
    s_nex_dat %<>% 
      dplyr::select(trip_id, person_id, gridid_vis, gridid_vis2, ipw, cost, ipw, boat_access,
                    cons_flt, sp_flt, cons2l, sp2l, cons3l, sp3l, gam_cons, gam_sp, vott, ann_inc, age,
                    ex_times12m, resident,accom, geom_accom, geometry, use_long, use_lat) %>%
      distinct() %>% 
      as.data.frame()
...
```{r}
## fixing conglomerate
  rum_dat %<>% mutate(gridid_alt2 = ifelse(is.na(gridid_alt2), gridid_alt, gridid_alt2))
  rum_dat %<>% mutate(gridid_vis2 = ifelse(is.na(gridid_vis2), gridid_vis, gridid_vis2))
...
rum_dat %<>%
  ...
  mutate(choice_fct = as.factor(choice)) # choice

rum_dat %<>% mutate(ID = row_number()) 
tmp <- rum_dat %>% group_by(trip_id) %>% distinct(gridid_alt2, .keep_all = TRUE)
tmp %<>% mutate(choice2 = ifelse(gridid_alt2 == gridid_vis2, 1, 0))
tmp %<>% mutate(rum2 = 1)
rum_dat %<>% left_join(tmp[, c("ID", "rum2")], by = "ID")

tmp %<>% filter(choice2 == 1)
rum_dat %<>% left_join(tmp[, c("ID", "choice2")], by = "ID")
rum_dat %<>% mutate(choice2 = ifelse(is.na(choice2), 0, choice2))
rum_dat %<>% mutate(rum2 = ifelse(is.na(rum2), 0, rum2))


    #'[#VALIDATE: TRUE, should equal number of trips]
    sum(rum_dat$choice) == n_distinct(rum_dat$trip_id)
    sum(rum_dat$choice2) == n_distinct(rum_dat$trip_id)
    
# Adding in columns and grid id for conglomerate nex grids
...
rum_dat %<>%
        ...
        mutate(fc_flt = ifelse(cset %in% c("s_n", "s_e"), (cost*0.111)*nl_land_km, fc_flt)) %>% # calc fuel cost
...
av_tc <- rum_dat %>% group_by(cset, br_site) %>% summarise(av_tc = mean(fcflt_spgam))

rum_dat %<>% left_join(av_tc) 

rum_dat %<>% mutate(tc = ifelse(cset %in% c("b_e", "s_e"), fcflt_spgam, av_tc))
```
...
rum_dat %<>% 
  ...
  mutate(across(contains('gridid2_b'), replace_na, 0)) %>% 
  mutate(across(contains('gridid2_s'), replace_na, 0)) %>% 
  mutate(across(contains('rum2'), replace_na, 0))
