...
//  predict stdp, stdp  // calc error // calculate error for xb - this liune does work 
...
* final model - store full model output inc confidenec intervals
putexcel set nex.xlsx, replace
nlogit fchoice c.fcflt_spgam || cset: || gridid_alt:, case(trip_id) // nested logit
putexcel (A1) = etable
...
** Comparable fishing RUM
	...
	asclogit fchoice c.fcflt_spgam if cset == "b_e" [pweight = ipw], case(trip_id) alt(gridid_alt)  
