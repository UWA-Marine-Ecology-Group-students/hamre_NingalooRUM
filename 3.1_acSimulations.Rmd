...
# data
dat <- readRDS("./data/gpkg/2.1_acChoiceSet_4283.gpkg")  # this is the generalised data with static site attributes
...
b <- read_excel("./data/03_data/ac_mod.xlsx") %>% dplyr::select(Vars, Coef)
 
# covariance matrix
v <- read_excel("./data/03_data/ac_mod.xlsx", sheet = "v", col_names = rep("x", 8)) %>% 
  dplyr::select(-x...1 ) %>% 
  distinct()
...
# define vars used in RUM
     vars <- c("travelCost", "SZ1", "SZ2", "SZ3","SZ4", "SZ5")
...
# Summaries across draws
    ...
    W_impact_trip
    png("./plots/ASC RUM/ac_s1_impact_trip.png", height = 50*nrow(W_impact_trip), width =
...
ggsave(path = "./plots/ASC RUM", filename = paste("ac_sim1.presim.png", sep = ''), width = 8, height = 4)
...
ggsave(path = "./plots/ASC RUM", filename = paste("ac_sim1.postsim.png", sep = ''), width = 8, height = 4)
...
ggsave(path = "./plots/ASC RUM", filename = paste("ac_sim1.diff.png", sep = ''), width = 8, height = 4)
