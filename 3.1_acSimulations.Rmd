...
tmp <- dat %>% distinct(gridid_alt, .keep_all = TRUE)
table(tmp$cset)
```
...
# FISHING  - think when there are attributes
...
# ASC NEX RUM
    ...
    # coefficients matrix (mvn_b)? so we have 100 utility values for each variable - why? 
      ...
      # the code below often fails: non-confromable. This means that you need to check the names in the vectors above to make sure the are the same as the model and mach the data set in teh rught order
        for (i in 1:n1) { 
...
dat %<>%
  mutate(gridid_int = group_indices(., gridid_alt)) %>% # making id integers
  mutate(accom_int = group_indices(., accom)) # making id integers

# Probabilities
    Vj <- as.data.table(Vj) # make data.table to save RAM
    cols <- colnames(Vj) # save the names of the draws for later
    Vj$br <- as.integer(dat$accom_int) # run function by BR
    Vj$br <- as.integer(dat$ramp_id) # run function by BR
    Vj$ID <- as.integer(dat$gridid_int) # add gridID_alt for group_by
    Vj$cset <- dat$cset # add gridID_alt for group_by
    ...
    # applying probability function to every cell in Vj by boat ramp
    p <- Vj[ , lapply(.SD, f1), by = ID, .SDcols = c(cols)] # applying probability function to every cell in Vj by boat ramp
    ...
    # alternatives presented to the person - sum of exp(Vj)
     ...
     logsum_base <- Vj[ , lapply(.SD, sum), by = ID, .SDcols = cols]    # dat %<>% arrange(ramp_id, gridid_alt) # ordering
```
...
# Calculate probability of visit and logsums: post-simulation
    ...
    p <- Vj[ , lapply(.SD, f1), by = ID, .SDcols = c(cols, "ID")]
    p <- p[order(br, ID)]
    # p <- p[order(br, ID)]
    ...
    logsum_sim <- Vj[ , lapply(.SD, sum), by = ID, .SDcols = cols] # 18876 trips
...
# Extracting the welfare impact - EX
...
# Summaries across draws
    ...
    dat %<>%  mutate(rampid = as.integer(ramp_id))
  
    dat %<>% left_join(logsum_sim[ , c("br", "w.mean", "w.upp", "w.low")], by = c("rampid" = "br"))
    
    # dat %<>% left_join(logsum_sim[ , c("ID", "w.mean", "w.upp", "w.low")], by = c("ID" = "br"))
