...
rm(boat2L, boat3L, temp, gam_cons, sp_cons, cons_mod, sp_mod, tmp, bl_mod, lm_ann_inc, boat_perf, gam_sp)
```
...
# Make data stata friendly
    ...
    b_ex_dat %<>% 
      dplyr::select(trip_id, person_id, gridid_vis, ramp_name, ipw, cost, ipw, boat_access,
                    ...
                    geom_accom, geometry, use_long, use_lat) %>%
      distinct() %>% 
      as.data.frame()
...
# test expand
   table(rum_dat$to_rm) # 0: 94806 1: 20952 
...
rum_dat %<>% mutate(finish = ifelse(str_detect(gridid_alt, "b_"), geom_br, geom_cent)) 
...
Outputs <- gmapsdistance(
  origin = rum_dat$geom_accom,
  destination = rum_dat$finish,
  mode = "driving",
